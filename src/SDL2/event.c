...
int keydown_to_key88(SDL_Keysym keysym) {
  ...
  //SDL_TO_PC88_KEY(SDLK_PRINT, KEY88_COPY);				
  SDL_TO_PC88_KEY(SDLK_SCROLLLOCK, KEY88_KANA);				
  SDL_TO_PC88_KEY(SDLK_PAUSE, KEY88_STOP);				
  //SDL_TO_PC88_KEY(SDLK_BACKQUOTE, KEY88_STOP); // not 100% sure this is a good idea, but my mac doesn't have a pause key
  SDL_TO_PC88_KEY(49, KEY88_ZENKAKU);				
  ...
  SDL_TO_PC88_KEY(SDLK_LEFTBRACKET, KEY88_BRACKETLEFT);
  SDL_TO_PC88_KEY(SDLK_RIGHTBRACKET, KEY88_BRACKETRIGHT); 

  SDL_TO_PC88_KEY(SDLK_QUOTEDBL, KEY88_QUOTEDBL);
...
// FINISH KEYS HERE
  SDL_TO_PC88_KEY(SDLK_AT, KEY88_AT);
